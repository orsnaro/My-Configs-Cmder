...
@REM -----------------------------------------------
echo TESTINGG  all vars list : 
echo 1) %1 %2 %2
echo 2) is debug? !is_debug!
echo 3) is first post shell arg empty(1==true is empty)? !is_empty!
echo 4) !fstPoShellArg!
echo 5) !CD!
echo 6) !toCheckPath!
echo 7) !command_options!
echo END TESTING 
@REM -----------------------------------------------
...
if  exist "!toCheckPath!" ( 
	...
	@REM leaving the backslash at end of path will make it harder to use '~' or get dir name using other methods. so remove it!
		@REM (it's assured to be always the last char because we appended it our selfs before and removed any double quotes)
	...
	if exist "!toCheckPath!\.is_autoVenv" (
		...
		if [%fstPoShellArg%]==[] ( 
			echo %CD% 
				...
				if not %fstPoShellArg%==-1 (
					 if not %fstPoShellArg%==-i (
						 if not %fstPoShellArg%==-d (
							 cd /d "%fstPoShellArg%" 
						) 
					) 
				)
